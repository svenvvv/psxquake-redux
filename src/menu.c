...
static inline void M_DrawCharacter(int cx, int line, int num)
{
...
__attribute__((__always_inline__))
static inline void M_PrintWhite(int cx, int cy, char const *str)
{
    while (*str) {
...
__attribute__((__always_inline__))
static inline void M_DrawTransPic(int x, int y, qpic_t const *pic)
{
...
void M_ScanSaves(void)
{
    ...
    for (i = 0; i < MAX_SAVEGAMES; i++) {
        ...
        loadable[i] = false;
        int fmt_len = snprintf(name, sizeof(name), "%s/s%i.sav", com_gamedir, i);
        if (fmt_len < 0 || fmt_len >= sizeof(name)) {
            Sys_Error("M_ScanSaves: failed to format filename, %d\n", fmt_len);
        }
        f = fopen(name, "r");
        if (!f)
            continue;
